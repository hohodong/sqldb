...
import com.database.query.join.BNLJOperator;
import com.database.query.join.SNLJOperator;
...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        for(int i=0;i<indexColumns.size();i++){
            ...
            IndexScanOperator indexScanOperator = new IndexScanOperator(transaction, selectPredicate.tableName, selectPredicate.column, selectPredicate.operator, selectPredicate.value);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for(Map.Entry<Set<String>, QueryOperator> x : prevMap.entrySet()){
            for(JoinPredicate jp : joinPredicates){
                QueryOperator leftOp, rightOp;
                Set<String> tableSet = new HashSet<>();
                if(x.getKey().contains(jp.leftTable) && !x.getKey().contains(jp.rightTable)){
                    tableSet.add(jp.rightTable);
                    leftOp = x.getValue();
                    rightOp = pass1Map.get(tableSet);
                } else if(!x.getKey().contains(jp.leftTable) && x.getKey().contains(jp.rightTable)){
                    tableSet.add(jp.leftTable);
                    leftOp = pass1Map.get(tableSet);
                    rightOp = x.getValue();
                } else {
                    continue;
                }
                QueryOperator minCostQueryOperator = minCostJoinType(leftOp, rightOp,jp.leftColumn, jp.rightColumn);
                tableSet.addAll(x.getKey());
                result.put(tableSet, minCostQueryOperator);
            }
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        for(String tableName : tableNames){
            ...
            QueryOperator queryOperator = minCostSingleAccess(tableName);
            pass1Map.put(tableSet, queryOperator);
        }
        int maxTableSize=1;
        Map<Set<String>, QueryOperator> prevMap = pass1Map;
        while(maxTableSize < tableNames.size()){
            prevMap = minCostJoins(prevMap, pass1Map);
            for(Set<String> tablesSet : prevMap.keySet()){
                maxTableSize = Math.max(maxTableSize, tablesSet.size());
            }
        }
        int minCost = Integer.MAX_VALUE;
        QueryOperator retOperator = null;
        for(QueryOperator queryOperator : prevMap.values()){
            int curCost = queryOperator.estimateIOCost();
            if(curCost < minCost){
                retOperator = queryOperator;
                minCost = curCost;
            }
        }
        finalOperator = retOperator;
        addGroupBy();
        addProject();
        addSort();
        addLimit();
