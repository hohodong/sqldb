...
import com.database.common.iterator.ArrayBacktrackingIterator;
import com.database.common.iterator.BacktrackingIterator;
import com.database.table.PageDirectory;
...
public abstract class QueryOperator implements Iterable<Record> {
    protected QueryOperator source; // 当前操作符的输入源（上一个操作符）
    protected Schema outputSchema;
    protected TableStats stats; // 输出结果的统计信息（用于查询优化）
    ...
    /**
     * 功能：根据输入源（source）的模式和当前操作符的逻辑，计算输出模式。
     * 示例：
     *     Project 操作符会基于投影字段生成新的模式。
     *     Join 操作符会合并左右输入源的模式。
