...
public class LockContext {
    ...
    public void acquire(TransactionContext transaction, LockType lockType)
        ...
        LockContext lockContext = fromResourceName(lockman, getResourceName());
        ...
        if(lockType.equals(LockType.NL)){
            throw new InvalidLockException("the request is invalid because of NL LockType");
        }
    ...
    public void release(TransactionContext transaction)
        ...
        if(getNumChildren(transaction) > 0){
            throw new InvalidLockException("there are child locks");
        }
        if(parent != null){
    ...
    public void promote(TransactionContext transaction, LockType newLockType)
        ...
        if(LockType.substitutable(oldLockType, newLockType) && !newLockType.equals(oldLockType)){
            throw new InvalidLockException("requested lock type is not a promotion or promoting would cause the lock manager to enter an invalid state");
        }
        if(hasSIXAncestor(transaction) && (newLockType.equals(LockType.S) || newLockType.equals(LockType.IS) || newLockType.equals(LockType.IX))){
            throw new InvalidLockException("requested lock type is not a promotion or promoting would cause the lock manager to enter an invalid state");
        }
...
//        }
        ...
        if(newLockType.equals(LockType.SIX) && (oldLockType.equals(LockType.IS) || oldLockType.equals(LockType.IX))){
            ...
            lockman.acquireAndRelease(transaction, getResourceName(), newLockType, sisDesc);
        } else {
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if(curLockType.equals(LockType.NL)){
            throw new NoLockHeldException("transaction has no lock");
        } else if(curLockType.equals(LockType.IX) || curLockType.equals(LockType.X) || curLockType.equals(LockType.SIX)){
            newLockType = LockType.X;
        } else if(curLockType.equals(LockType.IS) || curLockType.equals(LockType.S)){
        ...
        for(Lock lock : lockList){
            if(lock.name.isDescendantOf(getResourceName())){
                if(lock.lockType.equals(LockType.IX) || lock.lockType.equals(LockType.X) || lock.lockType.equals(LockType.SIX)){
                    newLockType = LockType.X;
                } else if(lock.lockType.equals(LockType.IS) || lock.lockType.equals(LockType.S)){
                    newLockType = LockType.S;
                }
                releaseNames.add(lock.name);
                long curTransNum = transaction.getTransNum();
                numChildLocks.put(curTransNum, numChildLocks.get(curTransNum) - 1);
            }
        }
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        if(explicitLockType != LockType.NL || parent == null){
            if(explicitLockType.isIntent() && !explicitLockType.equals(LockType.SIX)){
                return LockType.NL;
            } else {
