...
class LeafNode extends BPlusNode {
    ...
    public Optional<RecordId> FindRid(DataBox key){
        Optional<RecordId> ret = Optional.empty();
        int n = keys.size();
        for(int i=0;i<n;i++){
            if(keys.get(i).compareTo(key) == 0){
                ret = Optional.of(rids.get(i));
                break;
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        int putPos=FindPutPos(key);
        if(putPos != keys.size() && keys.get(putPos).equals(key)){
        ...
        if(keys.size() > m*2){
            ...
            for(int i=m;i< keys.size();i++){
                newKeys.add(keys.get(i));
            }
            for(int i=m;i< rids.size();i++){
            ...
            if(rightSibling.isPresent()){
                newRightSibling = rightSibling;
            }

            LeafNode newLeafNode = new LeafNode(metadata, bufferManager, newKeys, newRids, newRightSibling, treeContext);

            Optional<Pair<DataBox, Long>> ret = Optional.of(new Pair<DataBox, Long>(keys.get(m), newLeafNode.getPage().getPageNum()));
            while(keys.size()>m){
                keys.remove(m);
            }
            while(rids.size()>m){
            ...
            rightSibling = Optional.of(ret.get().getSecond());
            sync();
            newLeafNode.sync();
            return ret;
    ...
    private int FindPutPos(DataBox key) {
        int ret = -1;
        int n = keys.size();
        for(int i=0;i<n;i++){
            if(keys.get(i).compareTo(key) >= 0){
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int m = (int) (metadata.getOrder()*2*fillFactor);
        while(data.hasNext()){
            Pair<DataBox, RecordId> curData = data.next();
            DataBox key = curData.getFirst();
            RecordId rid = curData.getSecond();
            int putPos= keys.size();
            if(!keys.isEmpty() && keys.get(putPos-1).equals(key)){
                throw new BPlusTreeException("duplicate put");
            }
        ...
        if(data.hasNext()){
            ...
            RecordId rid = curData.getSecond();
            int putPos= 0;
            newKeys.add(putPos, key);
            newRids.add(putPos, rid);
            Optional<Long> newRightSibling = Optional.empty();
            if(rightSibling.isPresent()){
                newRightSibling = rightSibling;
            }
            ...
            Optional<Pair<DataBox, Long>> ret = Optional.of(new Pair<DataBox, Long>(newKeys.get(0), newLeafNode.getPage().getPageNum()));
    ...
    public void remove(DataBox key) {
        int removePos = FindRemovePos(key);
        if(removePos == -1){
            return;
        }
    ...
    private int FindRemovePos(DataBox key) {
        int ret = -1;
        int n = keys.size();
        for(int i=0;i<n;i++){
            if(keys.get(i).compareTo(key) == 0){
