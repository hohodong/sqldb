...
import com.database.concurrency.LockUtil;
...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        if(root != null){
            ...
            int cursor=0;
            List<DataBox> curKeys = curLeafNode.getKeys();
            while(cursor<curKeys.size() && curKeys.get(cursor).compareTo(key)<0){
                cursor++;

            }
            if(cursor>=curKeys.size()){
                return Collections.emptyIterator();
            }
            return new BPlusTreeIterator(curLeafNode, cursor);
        }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if(oldRootRet.isPresent()){
            ...
            newKeys.add(oldRootRet.get().getFirst());
            newChildren.add(root.getPage().getPageNum());
            newChildren.add(oldRootRet.get().getSecond());
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        Optional<Pair<DataBox, Long>> oldRootRet = root.bulkLoad(data, fillFactor);
        while(oldRootRet.isPresent()){
            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChildren = new ArrayList<>();
            newKeys.add(oldRootRet.get().getFirst());
            newChildren.add(root.getPage().getPageNum());
            ...
            updateRoot(newRoot);
            oldRootRet = newRoot.bulkLoad(data, fillFactor);
        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator(LeafNode _curLeafNode, int _cursor){
            cursor = _cursor;
            curLeafNode = _curLeafNode;
        }
        ...
        public RecordId next() {
            ...
            if(cursor == curLeafNode.getRids().size()){
                if(!curLeafNode.getRightSibling().isPresent()){
                    throw new BPlusTreeException("what?");
                }
                curLeafNode = curLeafNode.getRightSibling().get();
                cursor=0;
            }
            int i=cursor;
            List<RecordId> elementData = curLeafNode.getRids();
