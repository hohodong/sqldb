...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        ...
        // InnerNode lastInnerNode = FindLastInnerNode(key);
        int pos = numLessThanEqual(key, keys);
        BPlusNode nxtNode = getChild(pos);

        return nxtNode.get(key);
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        BPlusNode nxtNode = getChild(0);

        return nxtNode.getLeftmostLeaf();
    }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int childPos = numLessThanEqual(key, keys);
        BPlusNode childNode = getChild(childPos);
        ...
        if(newNodeInfo.isPresent()){
            ...
            int putPos = numLessThanEqual(newKey, keys);
            keys.add(putPos, newKey);
            children.add(putPos+1,newChild);
            if(keys.size() > m*2){
                ...
                for(int i=m+1;i< children.size();i++){
                    newChildren.add(children.get(i));
                }

                InnerNode newInnerNode = new InnerNode(metadata, bufferManager, newKeys, newChildren, treeContext);

                Optional<Pair<DataBox, Long>> ret = Optional.of(new Pair<DataBox, Long>(keys.get(m), newInnerNode.getPage().getPageNum()));
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        int m = metadata.getOrder();
        int childPos = children.size()-1;
        BPlusNode childNode = getChild(childPos);
        Optional<Pair<DataBox, Long>> newNodeInfo = childNode.bulkLoad(data, fillFactor);
        while(newNodeInfo.isPresent()){
            // split
            DataBox newKey = newNodeInfo.get().getFirst();
            long newChild = newNodeInfo.get().getSecond();
            keys.add(keys.size(), newKey);
            ...
            childNode = getChild(childPos);
            newNodeInfo = childNode.bulkLoad(data, fillFactor);
