...
public class TestRecoveryManager {
    ...
    @Test
    public void testRestartTimeCost() {
        // 生成20个事务的复杂操作场景
        final int TRANSACTION_COUNT = 20;
        final int OPERATIONS_PER_TX = 50;  // 每个事务50次操作
        final int PAGE_VARIETY = 50;      // 使用100个不同的页面

        // 1. 创建更复杂的测试数据
        List<byte[]> dataVariants = new ArrayList<>();
        Random rand = new Random();
        for (int i = 0; i < 10; i++) {
            byte[] data = new byte[128];  // 128字节的页面数据
            rand.nextBytes(data);
            dataVariants.add(data);
        }
        ...
        // 模拟20个并发事务的复杂操作
        for (long txId = 1; txId <= TRANSACTION_COUNT; txId++) {
            Transaction tx = DummyTransaction.create(txId);
            recoveryManager.startTransaction(tx);
            activeTransactions.add(txId);

            // 每个事务执行多次混合操作
            for (int op = 0; op < OPERATIONS_PER_TX; op++) {
                // 随机选择页面和操作类型
                long pageNum = 10000000000L + (rand.nextInt(PAGE_VARIETY));
                short offset = (short) (rand.nextInt(10) * 128);  // 模拟对齐偏移

                // 生成随机前后镜像
                byte[] before = dataVariants.get(rand.nextInt(dataVariants.size()));
                byte[] after = Arrays.copyOf(before, before.length);
                after[rand.nextInt(after.length)] = (byte) rand.nextInt(256);

                // 记录页面写入日志
                LSNs.add(recoveryManager.logPageWrite(
                        txId, pageNum, offset, before, after
                ));

                // 随机插入事务状态变更
                if (op % 10 == 0) {
                    if (rand.nextBoolean()) {
                        LSNs.add(recoveryManager.commit(txId));
                    } else {
                        LSNs.add(recoveryManager.abort(txId));
                    }
                }
            }

            // 随机终止部分未完成的事务（模拟未提交的事务）
        ...
        // 3. 强制终止剩余事务（模拟崩溃时的未完成事务）
        for (Long txId : activeTransactions) {
            recoveryManager.logPageWrite(
                    txId,
                    10000000000L + rand.nextInt(PAGE_VARIETY),
                    (short) 0,
                    new byte[] {0x00},
                    new byte[] {0x01}
            );
...
//        ));
        ...
        System.out.println("平均恢复时间: " +
                totalElapsed / 1_000_000.0 + " ms");
    }
